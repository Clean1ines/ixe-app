...
(header_container, qblock), using an injected AssetDownloader.
"""
import logging
import re
...
# Убираем импорт utils.downloader
# from utils.downloader import AssetDownloader # УДАЛЯЕМ
# from domain.interfaces.html_processor import IHTMLProcessor # УДАЛЯЕМ - используем новый путь
from src.domain.interfaces.html_processing.i_html_processor import IHTMLProcessor # ИСПОЛЬЗУЕМ НОВЫЙ ИНТЕРФЕЙС

logger = logging.getLogger(__name__)

class ImageScriptProcessor(IHTMLProcessor): # Наследуемся от нового интерфейса
    """
    ...
    - Downloads images using the injected IAssetDownloader (via adapter)
    ...
    # async def process_html_block( # ИСПРАВЛЕНО: Убираем async, так как старые процессоры не были async
    def process_html_block( # ИСПРАВЛЕНО: Убираем async
        self,
        ...
        **kwargs
        ...
        # ИСПРАВЛЕНО: Используем 'downloader', который должен быть экземпляром OldStyleAssetDownloaderAdapter
        downloader_instance = kwargs.get('downloader') # Это будет OldStyleAssetDownloaderAdapter
        files_location_prefix = kwargs.get('files_location_prefix', '')

        if downloader_instance is None:
            raise ValueError("AssetDownloader adapter instance (OldStyleAssetDownloaderAdapter) must be provided in context via 'downloader' kwarg")
        ...
        for script in qblock_soup.find_all('script', string=re.compile(r"ShowPicture$$'[^']*'$$")):
            ...
            if match:
                ...
                try:
                    # ИСПРАВЛЕНО: Вызываем download у адаптера
                    # local_path = await downloader.download(img_url, assets_dir, asset_type='image') # СТАРОЕ
                    local_path = downloader_instance.download(full_img_url, assets_dir, asset_type='image') # НОВОЕ (sync call on adapter)
                    if local_path:
        ...
        for img_tag in qblock_soup.find_all('img', src=True):
            ...
            try:
                # ИСПРАВЛЕНО: Вызываем download у адаптера
                # local_path = await downloader.download(full_img_url, assets_dir, asset_type='image') # СТАРОЕ
                local_path = downloader_instance.download(full_img_url, assets_dir, asset_type='image') # НОВОЕ (sync call on adapter)
                if local_path:
    ...
    #     # Убираем этот метод, он не нужен для нового интерфейса IHTMLProcessor
    #     pass
